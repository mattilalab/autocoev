...
cat << EOF >> $TMP/$RESULTS/CAPS.G${RESGAPS}-I${RESIDEN}-B${RESBOOT}-P${PVALUE}-B${BONFERRONI}.xml
...
<graph label="AutoCoEv" directed="0" cy:documentVersion="3.0" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:cy="http://www.cytoscape.org" xmlns="http://www.cs.rpi.edu/XGMML">
  ...
  <att name="Filtering?" value="${GBLOCKS}" type="string" cy:type="String"/>
...
echo -e "Adding node $idxml $namexml $attribute in $TMP/$RESULTS/CAPS.G${RESGAPS}-I${RESIDEN}-B${RESBOOT}-P${PVALUE}-B${BONFERRONI}.xml"
cat << EOF >> $TMP/$RESULTS/CAPS.G${RESGAPS}-I${RESIDEN}-B${RESBOOT}-P${PVALUE}-B${BONFERRONI}.xml
...
# Add edges
sed 1d $TMP/$RESULTS/pairs-G${RESGAPS}-I${RESIDEN}-B${RESBOOT}-P${PVALUE}-B${BONFERRONI}.tsv | \
while read -r Name1 msa1 Name2 msa2 colA realA colB realB seq1 seq2 meanA meanB corr boot pvalA pvalB pMean corr1 corr2 bonferroni GapA GapB GapAB DivA DivB DivAB ; do
echo -e "Adding edge: $msa1-$msa2 in $TMP/$RESULTS/CAPS.G${RESGAPS}-I${RESIDEN}-B${RESBOOT}-P${PVALUE}-B${BONFERRONI}.xml"
cat << EOF >> $TMP/$RESULTS/CAPS.G${RESGAPS}-I${RESIDEN}-B${RESBOOT}-P${PVALUE}-B${BONFERRONI}.xml
  <edge id="$Name1-$Name2" label="$Name1 - $Name2" source="$msa1" target="$msa2" cy:directed="0">
    ...
    <att name="MSA column (species)" value="$seq1 - $seq2" type="string" cy:type="String"/>
  </edge>
EOF
done
echo "</graph>" >> $TMP/$RESULTS/CAPS.G${RESGAPS}-I${RESIDEN}-B${RESBOOT}-P${PVALUE}-B${BONFERRONI}.xml
else
