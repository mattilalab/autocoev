...
EPDIF="0.1" # p-values difference

# Let user change work dir here as well
echo -e "\n\e[96mChange the Work dir or press ENTER to accept:\e[39m"
read -e -i $TMP TMP

# Let user change work dir here as well
echo -e "\n\e[96mChange the RESULTS dir or press ENTER to accept:\e[39m"
read -e -i $TMP/$RESULTS TMPRES

# Automatically determine path to the file to be processed
ETSV="$TMPRES/pairs-P${PVALUE}-B${BONFERRONI}.tsv"
...
EOUT="filtered-P${PVALUE}-gaps${EGAPS}-pDiff${EPDIF}-Bonf${BONFERRONI}.tsv"
ENET="FILTERED-P${PVALUE}-gaps${EGAPS}-pDiff${EPDIF}-Bonf${BONFERRONI}.xml"

rm -rf $TMPRES/{$EOUT,$ENET}
rm -rf $TMPRES/pairs-P${PVALUE}-B${BONFERRONI}
mkdir -p $TMPRES/pairs-P${PVALUE}-B${BONFERRONI}

# Do not filter out pairs from "bad quality" MSA.
sed 1d "$ETSV" | while read -r Name1 msa1 Name2 msa2 colA realA colB realB seq1 seq2 flt1 flt2 meanA meanB corr boot corrT cCoev pvalA pvalB pMean pDiff corr1 corr2 bonferroni GapA GapB GapAB DivA DivB DivAB ; do
  ...
  #  echo "Pair from a good quality MSA"
    if (( $(echo "$GapA > $EGAPS" |bc -l) && $(echo "$GapB > $EGAPS" |bc -l) && $(echo "$pDiff < $EPDIF" |bc -l) )); then
      echo "$Name1 $msa1 $Name2 $msa2 $cCoev $pvalA $pvalB $pMean $bonferroni $boot" >> $TMPRES/pairs-P${PVALUE}-B${BONFERRONI}/$msa1-$msa2.tsv
...
cd $TMPRES/pairs-P${PVALUE}-B${BONFERRONI}
  for pair in * ; do
  ...
  BonferroniMEAN=$(awk '{print $9}' $pair | datamash mean 1)
   BonferroniMAX=$(awk '{print $9}' $pair | datamash max 1)
  BonferroniSDEV=$(awk '{print $9}' $pair | datamash sstdev 1)

  echo "$namesIDs $coevNumber $cCoevMIN $cCoevMAX $cCoevMEAN $cCoevSDEV $bootMIN $bootMEAN $bootMAX $bootSDEV $pMeanMIN $pMeanMAX $pMeanMEAN $pMeanSDEV $BonferroniMIN $BonferroniMAX $BonferroniMEAN $BonferroniSDEV" >> $TMPRES/$EOUT
  echo "Processed $pair"
done

sed -i "1i Name1 msa1 Name2 msa2 coevNumber cCoevMIN cCoevMAX cCoevMEAN cCoevSDEV bootMIN bootMEAN bootMAX bootSDEV pMeanMIN pMeanMAX pMeanMEAN pMeanSDEV BonferroniMIN BonferroniMAX BonferroniMEAN BonferroniSDEV" $TMPRES/$EOUT
